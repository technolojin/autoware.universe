  ...
  const float h, const float r, const uint16_t id, std::vector<GridCenter> & gnd_grids) const
{
  ...
  for (uint16_t idx = 1; idx < estimating_grid_num; ++idx) {
    ...
    curr_gnd_grid.avg_height = h;
    curr_gnd_grid.max_height = h;
    curr_gnd_grid.gradient = 0.0f;
  ...
  for (size_t i = 0; i < in_radial_ordered_clouds.size(); ++i) {
    ...
    if (i == 180) {
      ...
      for (const auto & grid : gnd_grids) {
        RCLCPP_INFO(
          this->get_logger(),
          "grid_id: %d, radius: %f, avg_z: %f, max_z: %f -- estimated_z: %f, gradient: %f",
          grid.grid_id, grid.radius, grid.avg_height, grid.max_height,
          grid.intercept + grid.radius * grid.gradient, grid.gradient);
      }
    }
  }
}
