...
#ifndef GROUND_SEGMENTATION__GRID_HPP_
#define GROUND_SEGMENTATION__GRID_HPP_
...
public:
    ...
    const float grid_size_m, const float grid_mode_switch_radius, const float virtual_lidar_z)
  {
    ...
    grid_size_rad_ = tier4_autoware_utils::normalizeRadian(
                       std::atan2(mode_switch_radius_ + grid_size_m_, virtual_lidar_z_)) -
                     tier4_autoware_utils::normalizeRadian(mode_switch_angle_rad_);
  ...
  uint16_t getGridId(const float radius) const
  {
    ...
    if (radius <= mode_switch_radius_) {
      ...
      auto gamma{tier4_autoware_utils::normalizeRadian(std::atan2(radius, virtual_lidar_z_), 0.0f)};
...
#endif  // GROUND_SEGMENTATION__GRID_HPP_
