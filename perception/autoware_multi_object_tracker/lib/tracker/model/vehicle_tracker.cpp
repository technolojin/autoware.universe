  ...
  const types::InputChannel & channel_info)
{
  ...
  if (measurement.shape.type == autoware_perception_msgs::msg::Shape::POLYGON) {
    types::DynamicObject object = measurement;
    const double tracker_yaw = motion_model_.getYawState();
    if (shapes::convertConvexHullToBoundingBox(measurement, tracker_yaw, object)) {
      object.kinematics.orientation_availability = types::OrientationAvailability::SIGN_UNKNOWN;
      object.shape.type = autoware_perception_msgs::msg::Shape::BOUNDING_BOX; // prevent infinite loop

      // call this function again
      conditionedUpdate(object, prediction, tracker_shape, measurement_time, channel_info);
      // return true to finish the function
      return true;
    } else {
