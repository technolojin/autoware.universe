...
TEST(isInsideBboxTest, Inside)
{
  ...
  bool result = autoware::image_projection_based_fusion::isInsideBbox(25.0, 25.0, roi);
  EXPECT_TRUE(result);
}
...
{
  const sensor_msgs::msg::RegionOfInterest roi = createRoi(20, 20, 10, 10);
  bool result = autoware::image_projection_based_fusion::isInsideBbox(20.0, 30.0, roi);
  EXPECT_TRUE(result);
}
...
{
  const sensor_msgs::msg::RegionOfInterest roi = createRoi(20, 20, 10, 10);
  bool result = autoware::image_projection_based_fusion::isInsideBbox(15.0, 15.0, roi);
  EXPECT_FALSE(result);
}
...
TEST(isInsideBboxTest, Zero)
{
  ...
  bool result = autoware::image_projection_based_fusion::isInsideBbox(0.0, 0.0, roi);
  EXPECT_TRUE(result);
}
