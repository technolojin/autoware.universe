...
def _build_tmux_launch_commands(
    launcher_paths: list[str], launch_args_cmd: list[str], launcher_pkg_install_dir: str
) -> list[tuple[str, str]]:
...
def create_tmux_launcher_script(
    ...
    launch_commands = _build_tmux_launch_commands(
    ...
    if not gui_terminal:
        lines.append(
            '[ -z "$TMUX" ] && [ -t 0 ] && [ -t 1 ] && exec tmux attach-session -t $SESSION_NAME 2>/dev/null || true'
        )
